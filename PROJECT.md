...
- **The arc page's header still has hardcoded fields Session 24 deliberately left out of scope.** `ARC`'s badges, `seasonPart`, and `dateRange` are still always Shibuya's regardless of `params.slug` — only `name` and `episodes` were switched to real per-arc data. None of this causes a wrong-series mismatch the way the pre-Session-24 bug did (badges/dateRange were never series-specific claims to begin with), but it's still a visible seam on the 4 non-Shibuya real arcs. (`stats` was fixed in Session 37 — see its own bullet below.)
