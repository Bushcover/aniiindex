...
// but a *different* verification technique, and this is deliberate, not an
// inconsistency (see Session 20's log entry for the full story):
//
// Session 19's first version of this function chained `.select().
// maybeSingle()` after the update, the same technique confirmContentItem
// still uses — but that only works when the row's *new* status is covered
// by the "Public read access on content_items" RLS policy, because
// Postgres filters UPDATE...RETURNING through the table's SELECT policy
// against the row's new values. confirmContentItem is fine (its outcomes
// are always 'pending' or 'confirmed', both already in that policy) but a
// successful flag's outcome, 'flagged', was never in it — so a genuinely
// successful flag and a blocked one both produced an empty RETURNING
// result, indistinguishable from the outside. Widening the read policy to
// include 'flagged' would fix that, but would also work against a real
// future need (the "moderation/flagged-items view" on the Phase 5+
// roadmap, which specifically wants to query flagged rows) — so instead of
// depending on that widening, this function verifies success a different
// way: read the row *before* the update (under the existing, unwidened
// policy — the same one confirmContentItem's own initial select already
// relies on), then read it again *after* with the exact same query. If the
// flag genuinely applied, the row's status is now 'flagged' and this
// second read finds nothing (RLS hides it, same as any other flagged row).
// If the write was silently blocked by RLS instead, the row's status is
// unchanged and the second read still finds it. This needs no SQL beyond
// what confirmContentItem already needs — no separate grant/policy
// dependency for flag versus confirm.
export async function flagContentItem(id) {
  ...
  const { data: after } = await supabase.from('content_items').select('status').eq('id', id).maybeSingle();

  if (after) {
    console.error(
      '[flagContentItem] row still readable (status still',
      after.status,
      ') for id',
      id,
      'after the flag update — likely blocked by RLS. Check that the Session 18 "grant update (confirmation_count, status)" + "Public can confirm or flag content_items" policy (PROJECT.md, Database schema section) has actually been run against this Supabase project.'
    );
    throw new Error(`Flag on content_items.id=${id} was not applied (likely blocked by RLS — see server logs).`);
  }
