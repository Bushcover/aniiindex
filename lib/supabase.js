...
export async function getArcMeta(arcSlug) {
  ...
  // Temporary diagnostic (Session 25) — a production report said
  // /arc/control-devil-arc renders no beats/content sections at all
  // despite the arc existing in Supabase with 5 real beats. This sandbox
  // has no live Supabase access to reproduce that directly, so this logs
  // exactly what this function resolves for a given slug (and any error)
  // to Vercel's function logs, to confirm or rule out a lookup/slug
  // mismatch as the cause. Remove once that bug is confirmed fixed — this
  // is intentionally temporary, unlike this file's other, permanent
  // module-load/error logs (see PROJECT.md's "Known issues" for why those
  // are kept and this one isn't meant to be).
  console.log('[getArcMeta] slug:', arcSlug, '| data:', data, '| error:', error);
...
export async function getArcsBySeries(anilistSeriesId) {
  const { data, error } = await supabase
    ...
    .eq('anilist_series_id', anilistSeriesId)
